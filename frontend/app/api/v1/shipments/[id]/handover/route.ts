...
export async function POST(
  ...
  try {
    ...
    const { operatorAddress, nextHandlerAddress, location, locationContext } = body;

    if (!operatorAddress || !nextHandlerAddress) {
      return NextResponse.json({ error: "Operator and next handler addresses are required." }, { status: 400 });
    }

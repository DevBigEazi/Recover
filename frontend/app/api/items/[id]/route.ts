...
export async function GET(request: Request, { params }: Props) {
  try {
    ...
    const publicItem = {
      ...
      contactInfo: item.status === "Lost" ? (item.contactInfo || [ownerUser?.phone, ownerUser?.whatsapp, ownerUser?.email].filter(Boolean).join(" | ") || null) : null,
      phone: item.status === "Lost" ? (item.phone || ownerUser?.phone || null) : null,
      whatsapp: item.status === "Lost" ? (item.whatsapp || ownerUser?.whatsapp || null) : null,
      email: item.status === "Lost" ? (item.email || ownerUser?.email || null) : null,

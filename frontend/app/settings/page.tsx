...
export default function SettingsPage() {
  ...
  const activeApiKeyMasked = apiKeyMasked || localApiKeyMasked || (apiKey ? `${apiKey.substring(0, 13)}••••${apiKey.slice(-4)}` : null);
  const activeTestApiKeyMasked = testApiKeyMasked || localTestApiKeyMasked || (testApiKey ? `${testApiKey.substring(0, 13)}••••${testApiKey.slice(-4)}` : null);
  ...
  const handleGenerateApiKey = async (keyType: "live" | "test" = "live") => {
    ...
    try {
      const res = await fetch("/api/profile/api-key", {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-owner-address": account.address },

...
export default function DashboardPage() {
  ...
  const { username, companyName, isProfileLoaded, hasPersonalProfile, hasMerchantProfile } = useProfile();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          ...
          {account && (
            <Link
              href="/register"
              className="inline-flex items-center justify-center bg-accent hover:bg-accent/90 text-neutral-white font-semibold rounded-lg px-4 py-2 text-xs sm:text-sm transition-colors cursor-pointer shadow-xs shrink-0"
            >
              + Register New Item
            </Link>
          )}
        ...
        {isAuthLoading ? (
          ...
          /* Merchant-Only User Activation Gate */
          <div className="bg-neutral-white border border-neutral-mist rounded-2xl shadow-xs p-8 sm:p-12 text-center max-w-lg mx-auto mt-8 space-y-5 animate-fade-in">
            <div className="w-14 h-14 rounded-2xl bg-accent/10 border border-accent/20 text-accent flex items-center justify-center mx-auto">
              <ShieldCheck className="w-7 h-7" />
            </div>
            <div className="space-y-2">
              <h2 className="text-xl font-bold text-primary font-display">
                Activate Your Personal Items Vault
              </h2>
              <p className="text-xs sm:text-sm text-neutral-slate max-w-md mx-auto leading-relaxed">
                Your account is currently active in Business Mode ({companyName || "Store"}). Complete a quick personal profile setup to start registering your personal belongings and generating scannable QR stickers.
              </p>
            </div>
            <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
              <Link
                href="/settings"
                className="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-accent hover:bg-accent/90 text-neutral-white text-xs font-bold transition-all shadow-xs cursor-pointer text-center"
              >
                Set Up Personal Profile
              </Link>
              <Link
                href="/workspace"
                className="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-neutral-mist hover:bg-neutral-mist/80 text-primary text-xs font-medium transition-colors border border-neutral-mist text-center"
              >

...
import { CheckCircle2, Copy, Share2, Download, PlusCircle, X, Loader2, AlertCircle, Calendar, User, FileImage } from "lucide-react";
...
export default function ReceiptIssuedModal({
  ...
  const { workspaceSession } = useTeam();

  const resolvedMerchantName =
    receipt.merchantName || workspaceSession?.merchantName || companyName || "Business";
  const resolvedMerchantLogo =
    receipt.merchantLogo || workspaceSession?.businessLogo || businessLogo;
  const resolvedMerchantPhone =
    receipt.merchantPhone || phone || whatsapp || null;
  const resolvedMerchantEmail =
    receipt.merchantEmail || email || null;
  ...
  const handleWhatsAppShare = () => {
    const text = `Here is your verified receipt for ₦${receipt.total.toLocaleString()} from ${resolvedMerchantName}: ${publicUrl}`;
        ...
        <div className="p-5 overflow-y-auto space-y-4">
          ...
          <div className="p-4 bg-slate-950/80 rounded-xl border border-slate-800 flex flex-col items-center text-center">
            ...
            <div className="w-10 h-10 rounded-xl border border-slate-700 bg-white flex items-center justify-center overflow-hidden p-1 mb-2">
              ...
              <img
                src={resolvedMerchantLogo || "/logo-icon.svg"}
              ...
              <div className="flex justify-between items-center text-slate-400 text-xs pt-1">
                ...
                <span className="text-slate-200 font-medium">
                  {receipt.issuedBy.role === "owner" || receipt.issuedBy.name === "CEO"
                    ...
                    ? "By: Manager"
                    : `By: ${receipt.issuedBy.name || "Staff"}`}
                  {receipt.issuedBy.branchName ? ` (${receipt.issuedBy.branchName})` : ""}
                </span>
              </div>
            )}
        ...
        <div className="p-4 bg-slate-950 border-t border-slate-800 flex flex-col gap-2.5">
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className="flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl border border-emerald-800/60 bg-emerald-950/80 hover:bg-emerald-900/80 text-xs font-semibold text-emerald-300 transition-all cursor-pointer min-h-10"
            >
            ...
            <button
              ...
              className="flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl border border-slate-800 bg-slate-900 hover:bg-slate-800 text-xs font-semibold text-slate-200 transition-all cursor-pointer min-h-10"
            >
            ...
            <button
              ...
              className="flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl border border-slate-800 bg-slate-900 hover:bg-slate-800 text-xs font-semibold text-slate-200 transition-all cursor-pointer disabled:opacity-50 min-h-10"
            >
              {isDownloadingPdf ? (
          ...
          <button
            type="button"
            onClick={onNewSale}
            className="w-full flex items-center justify-center gap-1.5 py-3 px-4 rounded-xl bg-blue-600 hover:bg-blue-500 text-xs font-bold text-white shadow-xs transition-all cursor-pointer min-h-11"
          >

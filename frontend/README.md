...
- `lib/`: Mongoose database schemas (`db.ts`), Stripe checkout client (`stripe.ts`), Currency FX engine (`currency.ts`), Web Push service utilities (`push.ts`), and AI engine helper (`ai.ts`).
...
- **Stripe & Adaptive Multi-Currency Engine**: Dynamic FX conversion (`lib/currency.ts`) formatting subscription plans ($15, $45, $100/mo) and report unlocks into local currencies.

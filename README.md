...
| **`/shipments`** | **Logistics Merchant Dashboard**: Commercial dispatch portal for creating tamper-proof package shipments (`PKG-XXXXXX`), scratch-off inner secret generation (`RCVR-XXXX`), log custody handovers to dispatch riders, dual WhatsApp link generation, subscription plan paywall cards, and dispatch logs. |
